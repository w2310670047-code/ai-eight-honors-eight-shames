...
它不做别的事：在代理动手写代码、调用不熟悉的接口、判断业务规则、重构既有实现、以及准备说"已完成"之前，提供一组**可核对**的判据，用来挡住瞎猜、臆想、越权重构、跳过验证和假装理解；并在每次有改动的执行结束后，要求留下两份固定报告（第 9 条：`执行报告-首次运行.md` 与 `执行报告-变更记录.md`）。
...
> 第 9 条（本仓库追加的交付纪律）
>
> 以交付无踪为耻，以留痕成报为荣 —— 报告**固定两个文件**、放在**项目根**：`执行报告-首次运行.md`（首次在该目录工作时写一次：项目基线与每个文件的主要内容及作用）与 `执行报告-变更记录.md`（每次新增或修改操作追加一条，**把「用户输入」与「做出的更改」成对精简记录**）。有改动才写；不刷时间戳文件；是否入库由人类决定。

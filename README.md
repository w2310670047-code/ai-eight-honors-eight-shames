...
> 第 9 条（本仓库追加的交付纪律）
>
> 以交付无踪为耻，以留痕成报为荣 —— 报告**固定两个文件**、放在**工作区根**。**工作区根怎么定（优先级从高到低，不可颠倒）**：① 该目录有 `.git`/`package.json` 等项目标记、或已有报告 → 就用它；② 它只是容器而任务明确指向某子目录 → 用那个子目录；③ 判不出来才列**一次**一级目录兜底（不递归）。文件内容：`执行报告-首次运行.md`（**先查找、没有才创建**。**读取优先级：首次运行.md ＞ 变更记录.md ＞ 实际目录结构**。文件在 → 读它当唯一基线、**严禁不必要地全量扫描**；文件不在 → **受限扫描**（跳过 `node_modules`/`.git`/`dist`/`build`/`.vscode` 等）后才建立。顶部必须记 **最后一次核对时间 / 核对文件数量 / 一级目录**；一级目录不符、或核心依赖文件比报告新 → **强制局部刷新并更新报告**。**且必须含「已知坑点（跨实例警示）」一节**：只列至今仍有效的坑，每条写清「坑是什么 / 怎么触发 / 正确做法」，并在顶部元数据区加一行指针。**首次运行报告走「体积软触发 + 职责划分」：任务启动时看一次体积，超过 ≈30 KB / ≈200 行就在本次任务提一次整理建议、等人类确认后再动；它只写当前有效内容——版本表只留最近 3 个 tag + 一行指向 `gh release list`，已失效坑点在下次刷新时删除（变更记录里已留痕）**）与 `执行报告-变更记录.md`（**有界读：最新条目在最前，只读到覆盖核对时间的那一条，不读全文**；**不设体积上限，超过 ≈256 KB 才考虑归档**；**用户每次提出需求或问题**都追加一条，把「用户输入」与「做出的更改」成对精简记录；没改动业务文件就写明「除本报告外，本次对话未对任何文件进行修改」——两份报告是**元机制**，不计入业务文件修改）。不刷时间戳文件；**只存本地、不入库**（把 `执行报告-*.md` 加进 `.gitignore`，提交代码时排除这两个文件）。
...
工作区根怎么定：① 有 `.git`/`package.json` 等标记或已有报告 → 用它；② 只是容器而任务指向某子目录 → 用子目录；③ 判不出来才列一次一级目录兜底。首次任务启动先查找 `执行报告-首次运行.md`：有就以它加变更记录为唯一基线、不再全量扫描；没有才受限扫描（跳过 node_modules/.git/dist/build/.vscode 等）后建立。每次有新需求或提问都留一条简要记录；有改动就记改动，没改动业务文件就写明「除本报告外，本次对话未对任何文件进行修改」（两份报告是元机制，不计入业务文件修改）。报告顶部记核对时间与文件数，过期就局部刷新。`执行报告-变更记录.md` **有界读**：最新条目在最前，只读到覆盖核对时间的那一条，不读全文。`执行报告-首次运行.md` 超过 **≈30 KB / ≈200 行**时本次任务提一次整理建议、等人类确认；它只写当前有效内容（版本表只留最近 3 个 tag + 一行指向 `gh release list`，已失效坑点下次刷新时删）。变更记录不设体积上限，超过 ≈256 KB 再考虑归档。首次运行报告还必须含「已知坑点（跨实例警示）」一节：只列至今仍有效的坑，每条写清坑是什么 / 怎么触发 / 正确做法，并在元数据区加一行指针。
```

...
> 第 9 条（本仓库追加的交付纪律）
>
> 以交付无踪为耻，以留痕成报为荣 —— 报告**固定两个文件**、放在**项目根**：`执行报告-首次运行.md`（**先查找、没有才创建**：进新工作区先找它，**有就读它当基线、不再全量扫工作区**；没有才完整读取工作区并建立；内容是项目基线与每个文件的主要内容及作用）与 `执行报告-变更记录.md`（**用户每次提出需求或问题**都追加一条，把「用户输入」与「做出的更改」成对精简记录；没改任何文件就写明「本次对话未对任何文件进行修改」）。不刷时间戳文件；是否入库由人类决定。
...
每次有新需求或提问都留一条简要记录；有改动就记改动，没改任何文件就写明「本次对话未对任何文件进行修改」。首次进新工作区先查找 `执行报告-首次运行.md`：有就读它当基线，没有才完整读取工作区并建立。
```

...
> 第 9 条（本仓库追加的交付纪律）
>
> 以交付无踪为耻，以留痕成报为荣 —— 报告**固定两个文件**、放在**项目根**：`执行报告-首次运行.md`（首次在该目录工作时写一次：项目基线与每个文件的主要内容及作用）与 `执行报告-变更记录.md`（**用户每次提出需求或问题**都追加一条，把「用户输入」与「做出的更改」成对精简记录；没改任何文件就写明「本次对话未对任何文件进行修改」）。不刷时间戳文件；是否入库由人类决定。
...
每次有新需求或提问都留一条简要记录；有改动就记改动，没改任何文件就写明「本次对话未对任何文件进行修改」。
```
